...
namespace MaximeLEAU\JobeetBundle\Tests\Repository;
...
class JobRepositoryTest extends WebTestCase
{
	...
	{
		$query = $this->em->createQuery('SELECT c FROM MaximeLEAUJobeetBundle:Category c');
		$categories = $query->getResult();

		foreach($categories as $category) {
			$query = $this->em->createQuery('SELECT COUNT(j.id) FROM MaximeLEAUJobeetBundle:Job j WHERE j.category = :category AND j.expires_at > :date');
			$query->setParameter('category', $category->getId());
			$query->setParameter('date', date('Y-m-d H:i:s', time()));
			$jobs_db = $query->getSingleScalarResult();

			$jobs_rep = $this->em->getRepository('MaximeLEAUJobeetBundle:Job')->countActiveJobs($category->getId());
	...
	public function testGetActiveJobs()
	{
		...
		foreach ($categories as $category) {
			$query = $this->em->createQuery('SELECT COUNT(j.id) from MaximeLEAUJobeetBundle:Job j WHERE j.expires_at > :date AND j.category = :category');
			$query->setParameter('date', date('Y-m-d H:i:s', time()));
			$query->setParameter('category', $category->getId());
			$jobs_db = $query->getSingleScalarResult();

			$jobs_rep = $this->em->getRepository('MaximeLEAUJobeetBundle:Job')->getActiveJobs($category->getId(), null, null);
			...
			if($jobs_db > 2 ) {
				$jobs_rep = $this->em->getRepository('MaximeLEAUJobeetBundle:Job')->getActiveJobs($category->getId(), 2);
				...
				$jobs_rep = $this->em->getRepository('MaximeLEAUJobeetBundle:Job')->getActiveJobs($category->getId(), 2, 1);
	...
	{
		$query = $this->em->createQuery('SELECT j FROM MaximeLEAUJobeetBundle:Job j WHERE j.expires_at > :date');
		$query->setParameter('date', date('Y-m-d H:i:s', time()));
		$query->setMaxResults(1);
		$job_db = $query->getSingleResult();

		$job_rep = $this->em->getRepository('MaximeLEAUJobeetBundle:Job')->getActiveJob($job_db->getId());
		...
		$job_rep = $this->em->getRepository('MaximeLEAUJobeetBundle:Job')->getActiveJob($job_expired->getId());
	...
	public function testGetForLuceneQuery()
	{
		...
		$jobs = $em->getRepository('MaximeLEAUJobeetBundle:Job')->getForLuceneQuery('FOO6');
		...
		$jobs = $em->getRepository('MaximeLEAUJobeetBundle:Job')->getForLuceneQuery('position:FOO7');
